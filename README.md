...
| 请求历史 | 自动记录每条请求的账号、实际渠道、耗时、尝试序列、缓存命中率（cached/prompt，取自上游 usage；最近 100 条，含流式） |
...
流式请求会自动向上游补 `stream_options.include_usage=true` 以获取 usage（响应中会多一个标准 usage 尾块），缓存命中率据此记录。
